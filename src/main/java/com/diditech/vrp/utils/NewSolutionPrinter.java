 ...
 */
public class NewSolutionPrinter {
    ...
    private static void printVerbose(PrintWriter out, VehicleRoutingProblem problem, VehicleRoutingProblemSolution solution) {
        ...
        for (VehicleRoute route : list) {
            ...
            for (TourActivity act : route.getActivities()) {
                ...
                costs += c;
                double arrTimeD = act.getArrTime();
                double endTimeD = act.getEndTime();
                Object arr;
                Object end;
                if(arrTimeD > 4000000){
                    arr = DateUtil.formatDateTime(new Date((long) arrTimeD));
                } else {
                    arr = Math.round(arrTimeD);
                }
                if(endTimeD > 4000000){
                    end = DateUtil.formatDateTime(new Date((long) endTimeD));
                } else {
                    end = Math.round(endTimeD);
                }
                out.format(leftAlgin, routeNu, getVehicleString(route), act.getName(), jobId, arr,
                    end, Math.round(costs));
                prevAct = act;
            }
